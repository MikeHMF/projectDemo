...
- (void)initValues
{
    UIImageView* topBGImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, self.bounds.size.width, kHeightOfTopScrollView)];
    topBGImageView.image = [UIImage imageNamed:@"top_bg_white"];
    ...
    [self insertSubview:_rootScrollView atIndex:0];
...
- (void)layoutSubviews
{
    ...
    NSLog(@"%@",NSStringFromCGRect(_rootScrollView.frame));
}
 ...
 * @result
 */
- (void)buildUI:(BOOL)isShowFirstPage
{
    ...
    [self createNameButtons:isShowFirstPage];
 ...
 * @result
 */
- (void)createNameButtons:(BOOL)isFirstButtonSelected
{
...
{
    NSInteger x = sender.frame.origin.x + sender.frame.size.width/2 - 160;
    //后边不能空
    if (x >= (_topScrollView.contentSize.width - 320)) {
        x = _topScrollView.contentSize.width - 320;
    }
    //左边不能空
    if (x < kWidthOfButtonMargin) {
        x = 0;
    }
    ...
    if (sender.frame.origin.x - _topScrollView.contentOffset.x > self.bounds.size.width - (kWidthOfButtonMargin+sender.bounds.size.width)) {
        //向左滚动视图，显示完整tab文字
        [_topScrollView setContentOffset:CGPointMake(sender.frame.origin.x + 30 - (_topScrollView.bounds.size.width- (kWidthOfButtonMargin+sender.bounds.size.width)), 0)  animated:YES];
    }
    
    if ((sender.tag ==  _viewArray.count + 99) && (sender.frame.origin.x - _topScrollView.contentOffset.x > self.bounds.size.width - (kWidthOfButtonMargin+sender.bounds.size.width))) {
    ...
    if (sender.frame.origin.x - _topScrollView.contentOffset.x < kWidthOfButtonMargin) {
        ...
        [_topScrollView setContentOffset:CGPointMake(sender.frame.origin.x - kWidthOfButtonMargin - 30, 0)  animated:YES];
    }
